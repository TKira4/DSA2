...
V xMap<K, V>::put(K key, V value)
{
    ...
    temp.add(value);
    this->count++;
    this->ensureLoadFactor(capacity);
    return retValue;
}
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K))
{
    ...
    for(Entry* entry : temp){
        if(keyEQ(entry->key, key)){
            V oldvalue = entry->value;
            if(deleteKeyInMap) deleteKeyInMap(entry->key);
            temp.removeAt(i);
            this->count--;
            return oldvalue;
        }
...
bool xMap<K, V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V))
{
    ...
    int i = 0;
    for(Entry* entry : temp){
        if(keyEQ(key, entry->key) && valueEQ(value, entry->value)){
            if(deleteKeyInMap) deleteKeyInMap(entry->key);
            if(deleteValueInMap) deleteValueInMap(entry->value);
            temp.removeItem(entry, xMap<K, V>::deleteEntry);
            this->count--;
            return true;
        }
    }
...
{
    // YOUR CODE IS HERE
    DLinkedList<V> List_key;
    for(int i = 0; i < capacity; i++){
        DLinkedList<Entry*> temp = this->table[i];
        for(Entry* entry : temp){
            List_key.add(entry->key);
        }
    }
    return List_key;
}
...
{
    // YOUR CODE IS HERE
    DLinkedList<K> List_value;
    for(int i = 0; i < capacity; i++){
        DLinkedList<Entry*> temp = this->table[i];
        for(Entry* entry : temp){
            List_value.add(entry->value);
        }
    }
    return List_value;
}
...
{
    // YOUR CODE IS HERE
    DLinkedList<V> List_clash;
    for(int i = 0; i < capacity; i++){
        List_clash.add(this->table[i].size());
    }
}
